...
def generate_hl7_message(msg_id):
    ...
    pid.pid_11 = f"{fake.street_address()}^^{fake.city()}^{fake.state_abbr()}^{fake.zipcode()}^{fake.country_code()}"
    ...
    orc.orc_5 = random.choice(["CM", "CA", "DC", "HD"])  # Random order control code
